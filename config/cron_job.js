...
cron.schedule("*/30 * * * *", async () => {
  ...
  try {
    let result = await Item.find({ sold: false, expired: false }).select(
      "current_bid product_name details photos time_stamp"
    );
    result.map(async (i) => {
      const date = new Date();
      if (i.time_stamp <= date && i.current_bid.name !== "-none-") {
        await Item.findOneAndUpdate(i._id, { expired: true, sold: true }).catch(
          (err) => console.log(err)
        );
      } else if (i.time_stamp <= date) {
        await Item.findOneAndUpdate(i._id, { expired: true }).catch((err) =>
          console.log(err)
        );
      } else return i;
    });
...
cron.schedule("*/35 * * * *", async () => {
  ...
  try {
    let result = await Item.find({ sold: false, expired: true }).select(
      "current_bid product_name details photos time_stamp winner"
    );
    ...
    for (let it = 0; it < result.length; it++) {
      ...
      if (i.current_bid.name !== "-none-") {
        await Item.findOneAndUpdate(i._id, {
          sold: true,
          winner: {
            name: i.current_bid.name,
            _id: i.current_bid.user_id,
            amount: i.current_bid.amount,
          },
        ...
        sendMail.push({
          ...
          price: i.current_bid.amount,
          time_stamp: format.asString("dd/MM/yyyy", new Date(i.time_stamp)),
          email: user.email,
        });
      }
    }
