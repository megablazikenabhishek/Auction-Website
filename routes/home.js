...
router.get("/uploadItem", (req, res) => { res.render("upload"); });
...
router.post(
    "/uploadItem", uploader({useTempFiles : true}), async (req, res) => {
      let product = {
      ...
      try {
        if (req.files.photos.tempFilePath) {
          await cloudinary.uploader
              .upload(req.files.photos.tempFilePath,
                      (err, result) => { product.photos.push(result.url); })
